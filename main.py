...
async def adding_count(message: types.Message, state: FSMContext):
    try:
        async with state.proxy() as data:
            ...
            item = models.Item(name=data['name'], buy_sum=data['buy_sum'],
                               count=data['count'], current_sum=data['current'])
            item.save()
            await state.finish()
...
async def current(call: types.CallbackQuery):
    ...
    for item in items:
        ...
        txt += item.name + ' ('+str(round((round(current*0.87, 2)/item.buy_sum - 1)
                                          * 100, 2))+'%) ' + str(round(current*0.87, 2)) + ' pуб.\n\n'
    await bot.send_message(call.from_user.id, txt + 'Текущий баланс: ' + str(round(sum1, 2)) + ' руб.')

@dp.callback_query_handler(text='del_item')
async def deling(call: types.CallbackQuery):
    items = models.Item.select()
